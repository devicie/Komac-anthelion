...
use super::{
    analyze_installer::{analyze_sources, normalize_installer_inputs},
    error::{AnthelionError, AnthelionResult, ErrorCode},
    types::{
        AnalyzeInstallerRequest, AnalyzeInstallersRequest, AnalyzedArtifact, GitHubRelease,
        KomacOptions, PullRequest, PullRequestQuery, UpdatePackageRequest, UpdatePackageResult,
    },
...
impl Komac {
        ...
        let downloader = Downloader::new_with_concurrent_and_progress(concurrency, false)
            ...
            .map_err(|error| {
                AnthelionError::failure(
                    ErrorCode::ClientInitializationFailed,
                    Report::from(error).wrap_err("Failed to create installer downloader"),
                )
            })?;
        let github_token = options.github_token.or_else(|| {
    ...
    pub async fn analyze_installer(
        &self,
        request: AnalyzeInstallerRequest,
    ) -> napi::Result<AnalyzedArtifact> {
        analyze_sources(
            Arc::clone(&self.downloader),
            self.concurrency,
            normalize_installer_inputs(vec![request.installer]),
        )
        .await
        ...
        .ok_or_else(|| {
            AnthelionError::failure(
                ...
                eyre!("Installer analysis returned no result"),
            )
            .into()
        })
    }
    ...
    pub async fn analyze_installers(
        &self,
        request: AnalyzeInstallersRequest,
    ) -> napi::Result<Vec<AnalyzedArtifact>> {
        analyze_sources(
            Arc::clone(&self.downloader),
            self.concurrency,
            normalize_installer_inputs(request.installers),
        )
        .await
    ...
    pub async fn find_pull_request(
        ...
        self.require_github()?
            ...
            .map_err(|error| {
                AnthelionError::failure(
                    ErrorCode::PullRequestLookupFailed,
    ...
    pub async fn get_github_release_notes(
        ...
        self.require_github()?
            ...
            .map_err(|error| {
                AnthelionError::failure(
                    ErrorCode::ReleaseNotesFetchFailed,
...
impl Komac {
    fn require_github(&self) -> AnthelionResult<GitHub> {
        self.github.clone().ok_or_else(|| {
            AnthelionError::failure(
                ErrorCode::GitHubAuthRequired,
                eyre!("This operation requires githubToken in the Komac constructor"),
            )
        })
    }
}
