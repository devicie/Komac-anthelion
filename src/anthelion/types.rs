...
#[napi(object)]
pub struct AnalyzeInstallerRequest {
    /// Artifact to download and analyze.
    pub installer: Either<String, InstallerSource>,
}

/// Options for analyzing several installer artifacts in one concurrent operation.
#[napi(object)]
pub struct AnalyzeInstallersRequest {
...
pub struct UpdatePackageRequest {
    ...
    #[napi(
        ts_type = "string | { source: 'explicit'; value: string } | { source: 'display' | 'product' | 'file' }"
    )]
...
pub struct VersionSelection {
    #[napi(ts_type = "'explicit' | 'display' | 'product' | 'file'")]
    pub source: String,
